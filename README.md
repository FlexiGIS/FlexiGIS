...
*The folder 01_raw_input_data contains the file Oldenburg.poly, which is used when filtering the OSM planet data to only include the data of Oldenburg. This is achieved by Osmosis and executed by "make filter_OSM". The OSM planet data is filtered spatially to include Oldenburg and thematically to include only the power tag data. This folder should contain the OSM planet data file. It is not included in the present distribution of FlexiGIS because of its size. It can be downloaded from: http://ftp5.gwdg.de/pub/misc/openstreetmap/planet.openstreetmap.org/planet.* The the resulting filtered OSM power data is stored in folder 01_raw_input_data. An example is provided with the FlexiGIS code as file niedersachsen-latest.osm.pbf.
